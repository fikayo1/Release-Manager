...
The Vercel entrypoint `api/index.py` disables the lifespan polling scheduler. Scheduled work there runs only when Vercel Cron calls `/api/cron/scheduler`, which verifies `CRON_SECRET` and `x-vercel-cron` before forwarding to protected `POST /scheduler/tick`. Standalone `uvicorn src.app:app` retains the in-process UTC scheduler.
