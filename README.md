...
4. The Console project's Vercel Cron invokes `GET /api/cron/scheduler` once daily at `00:00 UTC`, which is compatible with Vercel Hobby. The route verifies Vercel's `User-Agent: vercel-cron/1.0` and `CRON_SECRET`, then forwards a trusted `POST /scheduler/tick` to the API project. **Scan now** is the normal on-demand trigger. A Vercel plan with more frequent Cron Jobs is required before relying on arbitrary five-field schedules in production.
