...
1. Import the repository into Vercel using the Next.js framework preset, with the repository root as the Root Directory. The root `package.json` declares the `frontend/` workspace and Next.js so Vercel can identify the framework; the configured install/build commands are `npm --prefix frontend ci` and `npm --prefix frontend run build`. The root `requirements.txt` supplies Python dependencies for `api/index.py`.
