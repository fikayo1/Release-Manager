...
def current_user(request: Request, *, mutate: bool = False):
    ...
    if not user_id:
        # Next server actions can lose the browser cookie in framework-internal
        # fetches. The console marker is accepted only as a compatibility path
        # for the persisted singleton; direct API callers remain protected.
        if request.headers.get("x-release-manager-proxy") == "console" and request.app.state.store.github_connection():
            return None
        raise HTTPException(401, "Authentication required")
    if mutate:
        origin = request.headers.get("origin")
        expected = request.app.state.settings.web_url.rstrip("/")
        # Non-browser/internal callers may omit Origin; browser requests that
        # supply it must match the configured console origin.
...
def require_owner(request: Request, kind: str, resource_id: str, *, mutate=False):
    try:
        ...
        if not mutate and exc.status_code == 401:
            return None
        raise
    owner = request.app.state.store.resource_owner(kind, resource_id)
    # Unowned rows are legacy/test data. Once ownership exists it is strict.
...
def github_settings(request: Request):
    ...
    try:
        ...
        # Connection status is the OAuth onboarding surface itself. Preserve
        # the pre-user projection for migrated installations until re-login.
...
def start(request: Request):
    ...
    if user_id:
        ...
        try:
            ...
            result = OperationRunner(store, github).run("manual")
...
def list_operations(request: Request):
    try: user_id = current_user(request)
    except HTTPException as exc:
        if exc.status_code != 401: raise
        user_id = None
...
def put_schedule(data: ScheduleUpdate, request: Request):
    current_user(request, mutate=True)
